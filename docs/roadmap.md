...
- Implement `mac-mlx` MiniCPM-V 4.6 adapter.
- Support direct video URL analysis where the backend can stream the source.
- Add ASR/subtitle ingestion with coverage checks.
- Reproduce the data-center Excel demo from a sanitized spec.
...
- Excel builder with formula checks and visual render checks.
